...
@ assumes all pointers are aligned to a 16 byte boundary
@ section size can also be zero
BootstrapReloc:
	cmp r1, r2
	ldmloia r0!, {r3-r6}
	stmloia r1!, {r3-r6}
...
ASM_FUNCTION start_itcm
	...
	mov r1, #0b00000010 @ Data Cacheable
	...
	@ Initialize bootalloc
	bl bootalloc_init


	@ Run ctors
	@ With LTO there should only be one but still run them all
	ldr r4, =__init_s
	ldr r5, =__init_e
	1:
		cmp r4, r5
